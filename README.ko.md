...
5. 아니면 개발 서버를 실행하여 웹사이트를 게시하기 전에 확인해 볼 수도 있습니다.

    ```sh
    # 기본 포트는 8080입니다.
    % serum server /path/to/project --port <포트>
    ```

    > 지금은 프로젝트를 수정한 후 개발 서버 프롬프트에서 `build` 명령을 입력하여 수동으로 프로젝트를 다시 빌드해야 합니다. 자동 빌드 기능은 곧 추가될 예정입니다.
