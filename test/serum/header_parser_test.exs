defmodule HeaderParserTest do
  ...
  describe "parse_header/4" do
    ...
    test "test all types" do
      ...
      assert map.dateval == {{2017, 6, 24}, {19, 53, 27}} |> Timex.to_datetime(:local)
    ...
    test "one required key are missing" do
      ...
      assert result == {:error, {"`strlst` is required, but it's missing", "test.txt", 0}}
      StringIO.close(sio)
    end
    ...
    test "multiple required keys are missing" do
      ...
      assert result ==
               {:error, {"`strlst`, `intlst` are required, but they are missing", "test.txt", 0}}
    ...
    test "no header" do
      ...
      assert result == {:error, {"header parse error: header not found", "test.txt", 0}}
      StringIO.close(sio)
    end
    ...
    test "unexpected eof" do
      ...
      assert result ==
               {:error, {"header parse error: encountered unexpected end of file", "test.txt", 0}}
    ...
    test "int parse error" do
      ...
      result = parse_header(sio, "test.txt", options(), [])
      assert result == {:error, {"header parse error: `intval`: invalid integer", "test.txt", 0}}
      StringIO.close(sio)
    end
    ...
    test "int list parse error" do
      ...
      result = parse_header(sio, "test.txt", options(), [])
      assert result == {:error, {"header parse error: `intlst`: invalid integer", "test.txt", 0}}
      StringIO.close(sio)
    end
    ...
    test "list of lists" do
      ...
      assert result ==
               {:error,
                {"header parse error: `intlst`: " <> "\"list of lists\" type is not supported",
                 "test.txt", 0}}
    ...
    test "weird value type" do
      ...
      assert result ==
               {:error, {"header parse error: `intlst`: invalid value type", "test.txt", 0}}
